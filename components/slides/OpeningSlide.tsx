    ...
    <div className="slide-container">
      ...
      <div className="slide-content relative z-10">
        ...
        >
          ...
          >
            What if the next industrial disaster isn't caused by fire, flood, or fraud...
            <br />
            <span className="text-transparent bg-gradient-to-r from-orange-400 to-red-400 bg-clip-text font-bold text-xl md:text-2xl">But by silence?</span>
          </motion.p>
        </motion.div>
        ...
        >
          ...
          <div className="relative bg-dark-800/60 backdrop-blur-sm border border-orange-500/30 rounded-2xl p-6">
            <p className="text-lg text-gray-200 mb-3 text-center">
              The cost of silence in industrial operations:
            </p>
            ...
            <div className="flex justify-center gap-6 mt-4">
              {[
                { color: 'bg-red-500', label: 'Downtime' },
                { color: 'bg-orange-500', label: 'Defects' },
                { color: 'bg-yellow-500', label: 'Inefficiency' }
              ].map((indicator, index) => (
                <motion.div 
                  key={index}
                  initial={{ scale: 0 }}
        ...
        >
          {[
            { stat: '60%', label: 'Manual Processes', color: 'text-red-400' },
            { stat: '90%', label: 'Data Never Seen in Time', color: 'text-orange-400' },
            { stat: '24hr', label: 'Average Detection Delay', color: 'text-yellow-400' }
          ].map((item, index) => (
            <motion.div
              key={index}
